locals {
    ...
    fleet.subnet.name => concat(
      [
        for num in range(fleet.num_clusters): {
            private-cidr = cidrsubnet(fleet.subnet.cidr, 11, num + 2000)
            svc-cidr =  cidrsubnet(fleet.subnet.cidr, 7, num + 96)
            pod-cidr = cidrsubnet(fleet.subnet.cidr, 2, num+1)
      }]
     )},
     {
      "${var.gke_config.subnet.name}" = [
        {
            svc-cidr =  var.gke_config.subnet.ip_range_svcs
            pod-cidr = var.gke_config.subnet.ip_range_pods
        }
     ]
     }
  )
  ...
  flattened_list_secondary_subnets = flatten ([
    for item in flatten([for fleet in var.fleets: local.secondary_subnets[fleet.subnet.name]]):
        values(item)
  ])
}
